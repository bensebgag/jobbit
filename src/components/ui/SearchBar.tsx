...
export default  function SearchBar ( ){
    const {keyWords,setKeyWords,setWilayaIDs,setSkillsIDs,setJobExperienceIDs,setJobTypeIDs,handleClickGatogrie}=useKeyWords();
...
const fileterBy=data||fileters;

  if (isLoading)return  <p>...Loading</p>
    if(isError)return <p>{error.message}</p>;
    return (
        <Container styleElement={"mt-20"}>
            <div ref={ref} className={`flex flex-wrap items-center gap-1 px-2  border-[1px] bg-white   rounded-lg relative ${isFocused?"border-blue-500 border-[2px]":"border-gray-400 "}`}>
                <Button onClick={()=>setOpen(true)} className={"text-white flex items-center gap-1 font-bold text-lg bg-green-700 hover:cursor-pointer  "} >
                   <ListFilter />
                    <span>FILTER</span>
                </Button>
                {keyWords?.map((keyword, index: number) => (
                    <div
                        className={"flex items-center gap-1 py-2 px-4  rounded-4xl bg-blue-500"}
                        key={index}>
                        <span className={"text-white text-sm "} >{keyword.name}</span>
                        <X onClick={()=>handleRemoveKeyWords(index,keyword,keyword.title)} className={"w-4 h-4 bg-blue-100 rounded-full text-blue-500 hover:cursor-pointer hover:bg-blue-50  "}/>
                    </div>))}
                <input onFocus={()=>handleFocus()}
                       className={"flex-1  py-4 rounded-lg outline-none  "}
                       onChange={handleSearchChange}
                       value={searchInput}
                       placeholder={"Type some keywords,select elements on the list or make a full text search"} />
                <AnimatePresence>
               {isFocused && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -10 }}
                        className="absolute overflow-y-scroll w-full top-15 p-6 left-0 bg-white z-20 border-[1px] border-b-gray-400 rounded-sm h-64"
                    >
                        <div className="flex flex-col gap-2">
                            <p className="text-gray-600 font-medium text-base">Contract type</p>
                            <ul className="px-4 text-sm flex flex-col text-gray-800 font-medium">
                                <li className="p-3 hover:bg-gray-200 hover:cursor-pointer">CDD (fixed term)</li>
                                <li className="p-3 hover:bg-gray-200 hover:cursor-pointer">CDD (fixed term)</li>
                                <li className="p-3 hover:bg-gray-200 hover:cursor-pointer">CDD (fixed term)</li>
                            </ul>
                        </div>
           ...
           <AnimatePresence>
            {
                ...
                <motion.div  initial={{ opacity: 0, y: -50 }}
                             animate={{ opacity: 1, y: 0 }} transition={{duration:0.2}}  exit={{ opacity: 0 }} className={"flex flex-col item-center gap-6 p-4 bg-white mt-4"}>
                <X onClick={()=>setOpen(false)} className={"self-end hover:cursor-pointer"} />
                <div className={"flex items-center justify-between"}>
                    {fileterBy.map((cateogrie, index) => (
                    <div className={"flex flex-col self-start gap-2"} key={index}>
                     <p className={"text-sm font-medium text-gray-950"}>{cateogrie.title}</p>

                        <div className={"grid grid-cols-2 gap-1"}>
                            <AnimatePresence>
                            {
                                cateogrie.filters.length>10 && !showAll[index]?cateogrie.filters
                                        .slice(0,9).map((item, index) => (
                                            <motion.div
                                                ...
                                                exit={{opacity:0,x:-10}}
                                                className={"rounded-full text-center p-2 text-white text-[10px] font-medium bg-gray-500 hover:cursor-pointer"}
                              ...
                              : cateogrie.filters.map((item, index) => (
                                <motion.div
                                  ...
                                  transition={{duration:0.1,delay:index*0.03}}
                                   exit={{opacity:0,x:-10}}
                                   onClick={() => test(item, cateogrie.title)}
                                    className={"rounded-full text-center p-2 text-white  text-[12px] font-medium bg-gray-500 hover:cursor-pointer"} key={index}>
                                    <span>{item.name}</span>
                                </motion.div>
                                ...
                                {
                                    cateogrie.filters.length > 10 && !showAll[index] && (
                                        <motion.div
                                            initial={{ opacity: 0, x: -20 }}
                                            animate={{ opacity: 1, x: 0 }}
                                            layout
                                            ...
                                            className={"text-green-700 flex items-center justify-center text-[12px] font-normal border-1 border-green-300 rounded-lg hover:cursor-pointer "}
                                        >
                                            <span>SHOW MORE...</span>
                                        </motion.div>
                                    )
                                }
                                {
                                    cateogrie.filters.length > 10 && showAll[index] && (
                                        <motion.div
                                            ...
                                            exit={{ opacity: 0, x: -10 }}
                                            onClick={() => toggleShowAll(index)}
                                            className={"text-green-700 flex items-center justify-center text-[12px] p-1 font-normal border-1 border-green-300 rounded-lg hover:cursor-pointer "}
                                        >
                                            <span>SHOW LESS...</span>
                                        </motion.div>
                                    )
