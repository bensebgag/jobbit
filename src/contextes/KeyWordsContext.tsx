...
export const KeyWordsContextProvider = ({ children }: KeyWordsProviderProps): JSX.Element => {
 ...
 console.log(filtersCat);
    const {data:jobFilters, isLoading, isError,error} = useQuery({
        queryKey: ["jobsFilter", wilayaIDs, skillsIDs, jobTypeIDs, jobExperienceIDs],
        queryFn:()=>fetchFilteredJobs(filtersCat)
    });



const dataFilter=jobFilters||testAllJobs;
    return (
        <KeyWordsContext.Provider value={{ keyWords, setKeyWords,handleClickGatogrie,filtersCat,dataFilter,showAll,setShowAll ,setJobTypeIDs,setJobExperienceIDs,setSkillsIDs,setWilayaIDs,isLoading,isError,error}}>
    {children}
...
export const useKeyWords = () => {
    ...
    if (!context) {
        throw new Error("useKeyWords must be used within a KeyWordsContextProvider");
    }
    return context;
};
